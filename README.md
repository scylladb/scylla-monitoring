...
#### Download your plugins locally
The dashboards uses the alertmanager data srouce plugin to display alerts, unfortunately, the plugin is not stable, we suggest that you download the plugin from [here](https://github.com/camptocamp/grafana-prometheus-alertmanager-datasource/releases), the monitoring stack uses the 0.0.6 version.

Place it into the plugin directory before uploading the grafana dashboards.
