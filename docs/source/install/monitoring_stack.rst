...
This document describes the setup of Scylla Monitoring Stack, based on `Scylla Prometheus API </reference/monitoring_apis/#prometheus>`_This link when clicked, leads to a 404 error message on Github.
...
* Follow the Installation Guide and install `docker`_ on the Scylla Monitoring Stack Server. This server can be the same server that is running Scylla Manager. Alternatively, you can :doc:`Deploy Scylla Monitoring Stack Without Docker <monitor_without_docker>` .
