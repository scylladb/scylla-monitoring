...
Options:
  ...
  -p port                        - Override the default port (10911)
  -H port                        - Override the default http-port (10912)
...
if [ -z "$THANOS_SC_PORT"  ]; then
    THANOS_SC_HTTP_PORT="10912"
fi
if [ -z "$HOST_NETWORK" ]; then
	PORT_MAPPING="-p $BIND_ADDRESS$THANOS_SC_PORT:10911 -p $BIND_ADDRESS$THANOS_SC_HTTP_PORT:10912"
	THANOS_SC_PORT="10911"
	THANOS_SC_HTTP_PORT="10912"
fi
...
docker run ${DOCKER_LIMITS["sidecar"]} -d $DOCKER_PARAM $USER_PERMISSIONS \
	...
	"--grpc-address=0.0.0.0:$THANOS_SC_PORT" \
	"--grpc-grace-period=1s" \
	"--http-address=0.0.0.0:$THANOS_SC_HTTP_PORT" \
