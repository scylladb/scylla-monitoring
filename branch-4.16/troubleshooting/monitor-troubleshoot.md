...
This document describes steps that need to be done to troubleshoot monitoring problems when using [Grafana/Prometheus](https://monitoring.docs.scylladb.com/branch-4.16/reference/monitoring-apis.md) monitoring tool.
...
See the [monitoring](https://monitoring.docs.scylladb.com/branch-4.16/install/monitoring-stack.md#docker-post-installation) Docker post-installation section.
...
More on start-all.sh [options](https://monitoring.docs.scylladb.com/branch-4.16/install/monitoring-stack.md).
...
1. If it is running, use `curl http://<scylla_node>:9100/metrics` (where <scylla_node> is a Scylla server IP) to fetch metric data from the `node_exporter`.  If curl does not return data, the problem is the connectivity between Scylla Monitoring Stack and Scylla server. Please check your IPs and firewalls.
...
If you are using a standalone Prometheus server, make sure to copy the Prometheus configuration and recording rules as describe in [install without docker](https://monitoring.docs.scylladb.com/branch-4.16/install/monitor-without-docker.md#install-prometheus).
